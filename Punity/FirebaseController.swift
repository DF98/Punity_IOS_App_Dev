...
/*
    ...
    func setUpListeners() {
        ...
        teamsRef?.whereField("name", isEqualTo: DEFAULT_TEAM_NAME).addSnapshotListener { querySnapshot,
            error in
    ...
    func parseHeroesSnapshot(snapshot: QuerySnapshot) {
        snapshot.documentChanges.forEach { change in
            ...
            let abilities = change.document.data()["abilities"] as! String
            print(documentRef)
            
            if change.type == .added {
                print("New Hero: \(change.document.data())")
                let newHero = SuperHero()
                newHero.name = name
